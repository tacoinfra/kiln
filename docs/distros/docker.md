...
```shell
DOCKER_CONTENT_TRUST=1 docker run  --tmpfs /tmp --network host --rm obsidiansystems/kiln:0.7.2 --pg-connection="host=localhost port=5432 dbname=postgres user=postgres password=mysecretpassword"
```

On macOS (Docker Desktop for Mac):

```shell
DOCKER_CONTENT_TRUST=1 docker run  --tmpfs /tmp -p 8000:8000 obsidiansystems/kiln:0.7.2 --pg-connection="host=host.docker.internal port=5432 dbname=postgres user=postgres password=mysecretpassword"
```
...
```shell
DOCKER_CONTENT_TRUST=1 docker run  --tmpfs /tmp --network host --rm obsidiansystems/kiln:0.7.2 ...
```
