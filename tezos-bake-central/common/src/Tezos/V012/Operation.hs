...
instance FromJSON OperationContents where
  parseJSON = withObject "Operation" $ \v -> do
    ...
    case kind of
      ...
      -- V013 operation kinds. We use it there so as not to create extra cross compat data types
      -- because of a single change, especially since these operation kinds are not used anywhere.
