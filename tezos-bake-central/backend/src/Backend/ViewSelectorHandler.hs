    ...
    runQueries window = do
      leftBiasedUnions <$> sequenceA
        ...
        , queryBakerAlert "ErrorLogMultipleBakersForSameBaker" ["publicKeyHash", "client", "worker"]
          ...
          window
        , queryAlert "ErrorLogUpgradeAvailable" ["id", "namedChain", "commit"] Nothing
          (\elId (tId, tNamedChain, tCommit) -> ErrorLogView_UpgradeAvailable tId $ ErrorLogUpgradeAvailable elId tNamedChain tCommit)
            window
        ]
