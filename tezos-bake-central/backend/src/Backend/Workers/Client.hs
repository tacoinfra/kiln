...
clientWorker
  ...
  -> NodeDataSource
  -> IO (IO ())
clientWorker delay appCfg nds = worker delay $ readMVar (_nodeDataSource_parameters nds) >>= \protoInfo -> runNoLoggingT $ runDb (Identity (_nodeDataSource_pool nds)) $ flip runReaderT (ClientWorkerContext appCfg nds) $ do
