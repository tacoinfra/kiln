...
bakerRightsWorker
  ...
  -> Int
  -> m (w ())
bakerRightsWorker appConfig nds rightsHistoryWindow = mkWorker $
  (<* waitForNewFinalHead) $ runLogger $ do
    db <- askPool
    chainId <- askChainId
    res :: Either KilnRpcError () <- runExceptT $ do
  ...
  where
    mkWorker act = worker' "bakerRightsWorker" $ runReaderT act (KilnEnv appConfig nds)
...
bakerWorker
  ...
  => AppConfig -> NodeDataSource -> Int
  -> m (w ())
bakerWorker appConfig nds rightsHistoryWindow = mkWorker $
  (<* waitForNewFinalHead) $ runLogger $ do
    db <- askPool
    res <- runExceptT $ do
  ...
  where
    mkWorker act = worker' "bakerWorker" $ runReaderT act (KilnEnv appConfig nds)
...
checkMissedOpportunities
  ...
  -> Bool
  -> RawLevel
  -> ExceptT KilnRpcError m (AppSerializable ())
  ...
  endorsingAlerts :: [AppSerializable ()]
      ...
      let
        ...
        endorserDelegates = blockCrossData
            (^..Genesis.block_operations . traverse . traverse . Oxford.operation_contents . traverse . Oxford._OperationContents_Endorsement . Oxford.operationContentsEndorsement_metadata . Oxford.endorsementMetadata_delegate)
            (^..Oxford.block_operations . traverse . traverse . Oxford.operation_contents . traverse . Oxford._OperationContents_Endorsement . Oxford.operationContentsEndorsement_metadata . Oxford.endorsementMetadata_delegate)
            thisBlock
