...
import "template-haskell" Language.Haskell.TH (conE, conT, mkName, nameBase)
...
import Rhyolite.Backend.Schema.Class
  (DefaultKeyIsUnique, DefaultKeyUnique, HasSingleConstructor, SingleConstructor,
  defaultKeyToKey, singleConstructor)
...
import Tezos.Types
import Tezos.Base.Types (Cycle, Round)
...
data NotifyTag a where
  ...
  NotifyTag_ShowLedger :: NotifyTag (SecretKey, Either Text (PublicKeyHash, Maybe Tez))
...
instance HasDefaultNotify (Id ErrorLogNeedToFinalizeUnstake)
...
instance HasNotification NotifyTag ErrorLogNeedToFinalizeUnstake where
  notification _ = mkBakerNotify BakerLogTag_NeedToFinalizeUnstake
...
instance NeverNull PeriodSequence
...
mkRhyolitePersist (Just "migrateSchema") [groundhog|
  ...
  - entity: ErrorLogNeedToFinalizeUnstake
    autoKey: null
    keys:
      - name: ErrorLogNeedToFinalizeUnstakeId
        default: true
    constructors:
      - name: ErrorLogNeedToFinalizeUnstake
        uniques:
          - name: ErrorLogNeedToFinalizeUnstakeId
            type: primary
            fields: [_errorLogNeedToFinalizeUnstake_log]
...
instance DefaultKeyId ErrorLogNeedToFinalizeUnstake where
  toIdData _ (ErrorLogNeedToFinalizeUnstakeIdKey eid) = eid
  fromIdData _ = ErrorLogNeedToFinalizeUnstakeIdKey
...
bakerLogAssume = \case
  ...
  BakerLogTag_NeedToFinalizeUnstake -> id
...
bakerLogDep = \case
  ...
  BakerLogTag_NeedToFinalizeUnstake -> depBakerAlert' ErrorLogNeedToFinalizeUnstake_bakerField
...
instance ArgDict c NotifyTag where
  type ConstraintsFor NotifyTag c =
    ...
    , c (Id ErrorLogNeedToFinalizeUnstake)
    ...
    , c (SecretKey, Either Text (PublicKeyHash, Maybe Tez))
  ...
  argDict = \case
    ...
    NotifyTag_ErrorLog t' -> case t' of
      ...
      LogTag_Baker t -> case t of
        ...
        BakerLogTag_NeedToFinalizeUnstake -> Dict
