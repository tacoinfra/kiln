...
reportNoBakerHeartbeatError cid eventDetail = do -- TODO: Only on non-deleted bakers
  ...
  case existingLog of
    Nothing -> do
      ...
      queueAlert $
        Alert False "Baker has not seen block for a while" $
...
clearNoBakerHeartbeatError cid = do -- TODO: Only on non-deleted bakers
  ...
  when (not $ null lids) $ queueAlert $
    Alert True "Resolved: Baker has now seen a block" $
  ...
  case existingLog of
    Nothing -> do
      ...
      for_ node' $ \node -> do
        ...
        queueAlert $ Alert False "Unable to connect to node" $
...
clearInaccessibleNodeError nodeId = when' (nodeNotDeleted nodeId) $ do
  ...
  when (not $ null lids) $ for_ node' $ \node -> do
    queueAlert $ Alert True "Resolved: Now able to connect to node" $
  ...
  case existingLog of
    Nothing -> do
      ...
      for_ node' $ \node -> do
        ...
        queueAlert $ Alert False "Node on wrong network" $
...
clearNodeWrongChainError nodeId = when' (nodeNotDeleted nodeId) $ do
  ...
  node' <- get $ fromId nodeId
  when (not $ null lids) $ for_ node' $ \node -> do
    queueAlert $ Alert True "Resolved: Node on right network" $
  ...
  case existingLog of
    Nothing -> do
      ...
      for_ node $ \n -> do
        ...
        queueAlert $ Alert False heading $
...
clearBadNodeHeadError nodeId = when' (nodeNotDeleted nodeId) $ do
  ...
  node <- get $ fromId nodeId
  when (not $ null lids) $ for_ node $ \n -> do
    queueAlert $ Alert True "Resolved: Node is in sync" $
